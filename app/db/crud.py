    ...
    for gene in top_genes:
        analysis_run.candidate_genes.append(
            CandidateGene(
                ...
                score=gene["score"],
                rank=gene["rank"],
                rwr_score=gene.get("rwr_score"),
                ml_probability=gene.get("ml_probability"),
                final_score=gene.get("final_score"),
                degree=gene.get("degree"),
                pagerank=gene.get("pagerank"),
                betweenness_centrality=gene.get("betweenness_centrality"),
                closeness_centrality=gene.get("closeness_centrality"),
                shortest_path_to_nearest_seed=gene.get("shortest_path_to_nearest_seed"),
                oncogene_neighbor_count=gene.get("oncogene_neighbor_count"),
                created_at=datetime.utcnow(),
            )
        )
...
def candidate_gene_to_response(candidate):
    return CandidateGeneResponse(
        ...
        rank=candidate.rank,
        rwr_score=candidate.rwr_score,
        ml_probability=candidate.ml_probability,
        final_score=candidate.final_score,
        degree=candidate.degree,
        pagerank=candidate.pagerank,
        betweenness_centrality=candidate.betweenness_centrality,
        closeness_centrality=candidate.closeness_centrality,
        shortest_path_to_nearest_seed=candidate.shortest_path_to_nearest_seed,
        oncogene_neighbor_count=candidate.oncogene_neighbor_count,
    )
